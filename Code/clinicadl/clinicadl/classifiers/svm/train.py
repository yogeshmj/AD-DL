...
parser.add_argument("--split", type=int, default=None, nargs="+",
                    help="List of splits to perform. Default behaviour will perform all splits.")
...
if __name__ == "__main__":
    from utils.classification_utils import commandline_to_json, SVMTester
    from utils.data_utils import load_data, save_data, save_additional_parameters

    commandline = parser.parse_known_args()
    options = commandline[0]
    if commandline[1]:
        print("Unknown arguments: %s" % parser.parse_known_args()[1])
    commandline_to_json(commandline, log_dir=options.output_dir)

    if options.split is None:
        options.split = np.arange(options.n_splits)
